...
const TaskList = React.forwardRef<{ sortTasks: () => void; getTasks: () => Task[] }, TaskListProps>(({ viewMode, filters, selectedWorkspaceId, onFiltersChange, onSort, onTasksChange }, ref) => {
  ...
  const isGroupExpanded = useCallback((groupName: string) => {
    ...
    // Note: Hide Completed filter is handled at the database level, so if it's enabled,
    // completed tasks won't be returned by the API at all
      ...
      <div className="p-2 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200 shadow-sm space-y-2">
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <div className="relative">
            <div
              className="text-sm rounded-md px-3 py-1.5 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 w-32 md:w-32 cursor-pointer bg-white hover:bg-gray-50 transition-colors min-h-[32px] flex items-center"
              onClick={(e) => {
                e.stopPropagation();
